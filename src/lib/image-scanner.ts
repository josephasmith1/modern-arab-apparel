 ...
 */
export async function scanAvailableImages(): Promise<ImageMetadata[]> {
 ...
 * Parse image metadata from filename and path
 */
function parseImageMetadata(relativePath: string, fullPath: string, stat: fs.Stats): ImageMetadata | null {
  ...
  let colorSlug = '';
  let imageType: 'main' | 'back' | 'lifestyle' = 'main';
  let lifestyleIndex: number | undefined;
 ...
 */
export async function scanProductImageData(): Promise<ProductImageData[]> {
  ...
  for (const file of jsonFiles) {
    try {
      ...
      productImageData.push({
        productSlug,
        shopifyImages: shopifyImages.map((img: any) => ({
          id: img.id,
          src: img.src,
          alt: img.alt,
 ...
 */
function extractExpectedLocalPaths(shopifyData: any): string[] {
  const paths: string[] = [];
 ...
 */
export async function generateImageAuditReport(): Promise<{
  availableImages: ImageMetadata[];
  productImageData: ProductImageData[];
  summary: {
    totalProducts: number;
    totalShopifyImages: number;
  ...
  const availableImageMap = new Map<string, ImageMetadata>();
